...
test("quotas: agent runs are counted from agent_runs; over the hourly limit the call is refused and one alert opens", () => {
  ...
  const usedRuns = quotaStatus().find((q) => q.quota === "agent_runs_hour")!.used;
  db.prepare("delete from settings where key = 'cfg:agentRunsPerHour'").run();
  db.prepare("insert into settings(key, value) values ('cfg:agentRunsPerHour', ?)").run(String(usedRuns + 2));
  try {
    ...
    assert.throws(() => checkAgentQuota("test"), /agent runs in the last hour/);
...
test("quotas: failed probe attempts count against the hourly probe limit", () => {
  const usedProbes = quotaStatus().find((q) => q.quota === "probes_hour")!.used;
  db.prepare("delete from settings where key = 'cfg:probesPerHour'").run();
  db.prepare("insert into settings(key, value) values ('cfg:probesPerHour', ?)").run(String(usedProbes + 3));
  try {
